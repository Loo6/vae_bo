...
# 1) physchem_biology 2) physchem 3) biology 4) pure_smiles
    ...
    def vectorize_sequence(self):
        vectorized_seqs = [[self.char2ind[char] for char in sf.split_selfies(smi)]for smi in self.data['SELFIES']]
        '''
                                vectorized_seqs = []
    ...
    def get_len(self):
        seq_len = [len(list(sf.split_selfies(smi))) for smi in self.data['SELFIES']]
