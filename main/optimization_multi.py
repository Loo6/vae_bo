...
parser.add_argument('--property1', type=str, default='HLGap')
parser.add_argument('--property2', type=str, default='Dipole')
...
def get_optimized_z_multi(z_input, model, property1, property2, maximize, step_size=0.1, num_steps=50):
    ...
    for epoch in tqdm(range(num_steps+1), desc='generating molecules'):
        loss = 0
        optimizer.zero_grad()
        pred_prop1 = model.predictor(z0)[:, prop_idx[property1]]
        pred_prop2 = model.predictor(z0)[:, prop_idx[property2]]
        if maximize:
            loss -= (torch.mean(pred_prop1) + torch.mean(pred_prop2))
        else:
            loss += (torch.mean(pred_prop1) + torch.mean(pred_prop2))
        z_processed.append(z0.clone())
        prop1.append(pred_prop1)
        prop2.append(pred_prop2)
        loss.backward()
        optimizer.step()
        losses.append(loss)
    return z_processed, prop1, prop2

z_optimized, prop_optimized1, prop_optimized2 = get_optimized_z_multi(z_input=z_tensor, model=model, property1=args.property1, property2=args.property2, maximize=args.maximize, step_size=0.01, num_steps=10)
...
for i in range(len(z_optimized)):
    for z_idx in range(z_optimized[i].size(0)):
        with torch.no_grad():
            ...
            prop_list1.append(prop_optimized1[i][z_idx])
            prop_list2.append(prop_optimized2[i][z_idx])

results_df = pd.DataFrame({'Steps': [i for i in range(0, 11) for _ in range(num_molecules)], 'idx':[idx for _ in range(0, 11) for idx in range(num_molecules)], 'SMILES': smiles_list, f'{args.property1}': [prop.item() for prop in prop_list1], f'{args.property2}': [prop.item() for prop in prop_list2]})
output_dir = "../result/optimization/multi"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)
if args.maximize:
    results_df.to_csv(f"../result/optimization/{args.property1}_{args.property2}_maximize.csv", index=False)
else:
    results_df.to_csv(f"../result/optimization/{args.property1}_{args.property2}_minimize.csv", index=False)