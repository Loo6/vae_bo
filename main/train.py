...
trainer = pl.Trainer(devices=args.num_devices, accelerator="gpu", strategy='ddp', max_epochs=args.epochs, logger=pl.loggers.CSVLogger('../logs', name='vae_prop_log'),
                    callbacks=[LearningRateMonitor(logging_interval="epoch"),ModelCheckpoint(monitor="val_loss", mode="min", save_top_k=10, dirpath='../checkpoints/vae_prop', save_last=True)])

print('Training..')
trainer.fit(model, dm_prop)
